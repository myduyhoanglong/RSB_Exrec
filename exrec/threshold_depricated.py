...
import numpy as np
from optimizer_depricated import *
...
def search_gamma_phi_threshold_fixed_loss(scheme, N, gamma, gamma_phi_start=1e-4, model=False):
    ...
    while (not cross or cnt < maxcnt) and gamma_phi > cutoff:
        ...
        if cross:
            cnt += 1
        curr_params, curr_ratio = optimize_fixed_noise(scheme, N, gamma, gamma_phi, model)
        print(gamma_phi, curr_params, curr_ratio)
        if ratio is not None and not cross and (curr_ratio >= 1 > ratio):
            cross = True
...
def optimize_fixed_noise(scheme, N, gamma, gamma_phi, model=False):
    ...
    if scheme == KNILL:
        init_pairs = [(3, 8, -0.1, -0.1), (3, 8, -0.3, -0.3), (5, 8, -0.1, -0.1), (5, 8, -0.3, -0.3)]
    elif scheme == HYBRID:
...
def optimize_fixed_noise_with_init_params(scheme, N, gamma, gamma_phi, init_pair):
    ...
    if scheme == KNILL:
        ...
        elif len(init_pair) == 4:
            (alpha_data, alpha_anc, offset_data, offset_anc) = init_pair
            init_params = [alpha_data, alpha_anc, offset_data, offset_anc]
            fixed_waiting = True
    ...
    try:
        if fixed_waiting:
            params, ratio = op.optimize_exrec_fixed_wait_time(scheme, init_params)
        else:
